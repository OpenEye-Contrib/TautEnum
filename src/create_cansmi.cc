...
std::string create_noncansmi( const OEChem::OEMolBase &in_mol ) {

  std::string smi;
  // default for OECreateSmiString is RGroups, Canonical and AtomMaps, but I don't think we'll
  // ever want the atom maps and they confuse matters.
  OEChem::OECreateSmiString( smi , in_mol , OEChem::OESMILESFlag::RGroups | OEChem::OESMILESFlag::AtomStereo | OEChem::OESMILESFlag::BondStereo );
  return smi;
